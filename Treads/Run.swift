...
class Run: Object {         //inherits from Object
    ...
    @objc dynamic public private(set) var id: String = ""
    @objc dynamic public private(set) var realmPath: String = "" //the realmPath for testing
    ...
    static func getAllRuns() ->Results<Run>? {
        ...
        do {
            ...
            print("Got the runs in getAllRuns \(runs) ")  //The path of the Realm Database
    ...
    static func getRun(byId id: String) -> Run? {
        do {
            ...
            print("In getRun in Run.swift: attempt to get a realm object that is being returned.")
    ...
    /* Create a function to create a copy of the default.realm file for testing - using this to
     resolve a problem whereby default.realm appears to be deleted each time*/
    /*
    static func saveRealmFile(realmFileURL: URL){
        ...
        do {
            try fileManager.copyItem(atPath: "\(realmFileURL)2.realm", toPath: "\(home)")
        }
        ...
        /*
        print("temporaryDirectory in saveRealmFile() is now: \(home)")
        let realmPath = realmFileURL                        //"~/Documents/"
        
        // get your playground URL
        let realmURL = home.appendingPathComponent("\(realmPath)")
        print("realmURL in saveRealmFile() is now: \(realmURL)")
         
         
        // First create a FileManager instance
        // if file is not exist create the file
         
        let newFile = realmURL.appendingPathComponent("copyDefaultRealm.realm").path
         
        if(!fileManager.fileExists(atPath:newFile)){
           fileManager.createFile(atPath: newFile, contents: nil, attributes: nil)
        }else{
            print("Realm File is already created")
        }
        */
    }
 */
